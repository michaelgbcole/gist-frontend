...
import synthesize from '@/util/synthesize-batch';
  ...
  try {
    ...
    const essays = await prisma.grade.findMany({
      where: {
        ...
        batchId: parseInt(req.body.batchId) // Assuming you're passing batchId in the request
      },
        select: {
        feedback: true
      }
    ...
    await prisma.batch?.update({
      ...
      data: {
        overallFeedback: synthesizedFeedback.join('')
      }
    });
